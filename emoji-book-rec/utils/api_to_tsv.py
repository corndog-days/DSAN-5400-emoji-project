    ...
    def open_library_api(self, query: str) -> List[Results]:
        ...
        if "docs" in data:
            for item in data["docs"]:
                books.append(Results( # Open Library does not provide all fields saved as "None"
    ...
    def iarchive_api(self, query):
        ...
        if "response" in data and "docs" in data["response"]:
            for item in data["response"]["docs"]:
                books.append(Results(
                    title=item.get("title", ""),
                    authors=", ".join(item.get("creator", [])) if isinstance(item.get("creator"), list) else item.get("creator", ""),
                    ...
                    averageRating=None,
                    ratingsCount=None,
                    language=", ".join(item.get("language", [])) if isinstance(item.get("language"), list) else item.get("language", "")
                ))
        return books
